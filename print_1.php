...
$db = new dbconnector("mine");
				...
				$school_query = "SELECT DISTINCT `schools`.*, `villages`.`villagename`, `cells`.`cellname`, `sector`.`sector_name`, `districts`.`district_name`
								 ...
								 `schools`.`school_id`='{$_GET['school_id']}' ";
								 //echo $school_query;
				$school = mysql_query($school_query);
				$sc = mysql_fetch_assoc($school);
				$year = date("Y",time());
				//select information of school
				$school_id_query = mysql_query("SELECT * FROM schools WHERE school_id='{$_GET['school_id']}'");
				$school_id = mysql_fetch_assoc($school_id_query);

				//select head master
				//var_dump($_SESSION);
				$head = mysql_query("SELECT * FROM users WHERE user_id='{$_GET['user_id']}'");
				$head_ = mysql_fetch_assoc($head);
				$tb = "
District:{$sc['district_name']}<br />
Sector:{$sc['sector_name']}<br />
Cell:{$sc['cellname']}<br />
Village:{$sc['villagename']}<br />
School: {$school_id['school_name']}<br /><br />
<div style='text-align:center; text-decoration:underline;'>Students who drop school in {$year}</div><br /><table border=1 style='border-collapse:collapse' width=100% cellspacing=0>";
	...
	$count = 0;
				...
				$tb .= "</table><br /><b>Total: ".count($students)." Student".(count($students)>1?"s":"")."</b><br /><br /><div style='width:300px; margin-right:5px; float:right;'>{$sc['sector_name']} Sector<br /><br /><br /><br />SEO<br />".$db->select1cell("users","Names",array("User_ID"=>$_SESSION['user_id']),true)."</div>";
				//echo $tb;
...
// 	}
// 	$tb .= "</table><br /><b>Total: ".count($students)." Student".(count($students)>1?"s":"")."</b><br /><br /><div style='width:300px; margin-right:5px; float:right;'>{$school_id['SchoolName']}<br /><br /><br /><br />SEO<br />".$db->select1cell("users","username",array("user_id"=>$_SESSION['user_id']),true)."</div>";
