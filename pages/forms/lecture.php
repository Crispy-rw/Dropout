...
$lect_id = $lect_name = $lect_grade = $dept_id =  "";
$lect_id_err = $lect_name_err = $lect_grade_err = $dept_id_err =  "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["lect_id"]))){
        $lect_id_err = "Please enter a Lecture Code.";
    } else{
        // Prepare a select statement
        $sql = "SELECT lect_id FROM lecture WHERE lect_id = ?";
        
        if($stmt = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("s", $param_lect_id);
            
            // Set parameters
            $param_lect_id = trim($_POST["lect_id"]);
            ...
            if($stmt->execute()){
                ...
                if($stmt->num_rows == 1){
                    $lect_id_err = "This Lecture Code is already taken.";
                } else{
     ...
     if(empty(trim($_POST["lect_name"]))){
        $lect_name_err = "Please Select Lecture Name.";     
    } else{
        $lect_name = trim($_POST["lect_name"]);
    }
    
      if(empty(trim($_POST["lect_grade"]))){
        $lect_grade_err = "Please Select Grade.";     
    } else{
        $lect_grade = trim($_POST["lect_grade"]);
    }
    // Validate dept_id
    if(empty(trim($_POST["dept_id"]))){
        $dept_id_err = "Please Select Head of Department .";     
    } else{
    ...
    if(empty($lect_id_err) && empty($lect_grade_err) &&empty($lect_name_err) && empty($dept_id_err)){
        
        // Prepare an insert statement
        $sql = "INSERT INTO lecture (lect_id,lect_name,dept_id,lect_grade) VALUES (?,?,?,?)";
         
        if($stmt = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("ssss", $param_lect_id, $param_lect_name,  $param_dept_id, $param_lect_grade);
            
            // Set parameters
            $param_lect_name = $lect_name;
            $param_lect_id = $lect_id;
            $param_dept_id = $dept_id; 
            $param_lect_grade = $lect_grade;
      ...
      <div class="row">
        ...
        <div class="box">
            <div class="box-header">
              <h3 class="box-title">Lecture Features </h3>
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                  <?php
                      ...
                      try{  
                          $sql = 'SELECT lecture.lect_id, lecture.lect_name,lecture.dept_id,lecture.lect_grade FROM lecture';

                         $query = $db->prepare($sql);

                         $query->execute(array('lect_id'=>$lect_id));
                         ...
                         if ($query->rowCount() > 0) {
                          ...
                          foreach ($dept as $row) 
                          {
                            ...
                            <tr>
                              ...
                              <td><?php echo htmlentities($row->lect_name); ?></td>
                              <td><?php echo htmlentities($row->dept_id); ?></td>
                              <td><?php echo htmlentities($row->lect_grade); ?></td>
                              <td>
                                <a href="#lecture_edit_<?php echo $row->lect_id; ?>" class="btn btn-success btn-sm" data-toggle="modal"><span class="fa fa-edit "></span> Edit </a>
                                <a href="#lecture_delete_<?php echo $row->lect_id; ?>" class="btn btn-danger btn-sm" data-toggle="modal"><span class="fa fa-trash"></span> Delete </a>
                              </td>
                             <?php include 'lecture_edit_delete_modal.php';?>
                            </tr>
                            <?php 
                            $i++;
                          }
              ...
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-9 <?php echo (!empty($lect_id_err)) ? 'has-error' : ''; ?>">
                    <input type="text" name="lect_id" class="form-control"  placeholder="Lecture Code">
                    <span class="help-block"><?php echo $lect_id_err; ?></span>
                  </div>
                </div> 

                <div class="form-group ">
                  <label for="inputEmail3" class="col-sm-3 control-label"> Lecture Name </label>

                  <div class="col-sm-9 <?php echo (!empty($lect_name_err)) ? 'has-error' : ''; ?>">
                   <input type="text" name="lect_name" class="form-control"  placeholder="Lecture Name">
                      <span class="help-block"><?php echo $lect_name_err; ?></span>
                </div>
              </div>

                <div class="form-group">
                  <label for="inputdept_id3" class="col-sm-3 control-label">Department Code </label>
                ...
                <div class="form-group">
                  <label for="inputdept_id3" class="col-sm-3 control-label"> Grade </label>

                  <div class="col-sm-9 <?php echo (!empty($lect_grade_err)) ? 'has-error' : ''; ?>">
                   <select name="lect_grade" class="form-control select2" style="width: 100%;">
                        <option selected="selected"> </option>
                        <option>Lecture</option>
                        <option>Assistant Lecture</option>
                        <option>Course Instructor </option>
                      </select>
                      <span class="help-block"><?php echo $lect_grade_err; ?></span>
                  </div>
                </div>
