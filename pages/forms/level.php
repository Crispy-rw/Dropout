...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["level_id"]))){
        $level_id_err = "Please enter a Level Code.";
    } else{
        // Prepare a select statement
        $sql = "SELECT level_id FROM level WHERE level_id = ?";
        
        if($stmt = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("s", $param_level_id);
            
            // Set parameters
            $param_level_id = trim($_POST["level_id"]);
            ...
            if($stmt->execute()){
                ...
                if($stmt->num_rows == 1){
                    $level_id_err = "This Level Code is already taken.";
                } else{
     ...
     if(empty(trim($_POST["level_name"]))){
        $level_name_err = "Please Select Level Name.";     
    } else{
    ...
    if(empty($level_id_err) && empty($level_name_err) ){
        
        // Prepare an insert statement
        $sql = "INSERT INTO level (level_id,level_name) VALUES (?,?)";
         
        if($stmt = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("ss", $param_level_id, $param_level_name);
            
            // Set parameters
            $param_level_name = $level_name;
      ...
      <div class="row">
        ...
        <div class="box">
            <div class="box-header">
              <h3 class="box-title">level Features </h3>
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                  <?php
                      ...
                      try{  
                          $sql = 'SELECT level.level_id, level.level_name FROM level';
                         ...
                         if ($query->rowCount() > 0) {
                          ...
                          foreach ($dept as $row) 
                          {
                            ...
                            <tr>
                              ...
                              <td><?php echo htmlentities($row->level_name); ?></td>
                              
                              <td>
                                <a href="#edit_<?php echo $row->level_id; ?>" class="btn btn-success btn-sm" data-toggle="modal"><span class="fa fa-edit "></span> Edit </a>
                                <a href="#delete_<?php echo $row->level_id; ?>" class="btn btn-danger btn-sm" data-toggle="modal"><span class="fa fa-trash"></span> Delete </a>
                              </td>
                             <?php include 'level_edit_delete_modal.php';?>
                            </tr>
                            <?php 
                            $i++;
                          }
              ...
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-9 <?php echo (!empty($level_id_err)) ? 'has-error' : ''; ?>">
                    <input type="text" name="level_id" class="form-control"  placeholder="level Code">
                ...
                <div class="form-group ">
                  ...
                  <div class="col-sm-9 <?php echo (!empty($level_name_err)) ? 'has-error' : ''; ?>">
                    <input type="text" name="level_name" class="form-control"  placeholder="level Name">

                      <span class="help-block"><?php echo $level_name_err; ?></span>
                </div>
              </div>
