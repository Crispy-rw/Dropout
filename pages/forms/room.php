...
$room_id_err = $room_name_err = $description_err =  "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["room_id"]))){
        $room_id_err = "Please enter a Room Code.";
    } else{
        // Prepare a select statement
        $sql = "SELECT room_id FROM room WHERE room_id = ?";
        
        if($stmt = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("s", $param_room_id);
            ...
            if($stmt->execute()){
                ...
                if($stmt->num_rows == 1){
                    $room_id_err = "This Room Code is already taken.";
                } else{
     ...
     if(empty(trim($_POST["room_name"]))){
        $room_name_err = "Please Select Room Name.";     
    } else{
    ...
    if(empty(trim($_POST["description"]))){
        $description_err = "Please Select Description.";     
    } else{
    ...
    if(empty($room_id_err) && empty($room_name_err) && empty($description_err)){
        
        // Prepare an insert statement
        $sql = "INSERT INTO room (room_id,room_name,description) VALUES (?,?,?)";
         
        if($stmt = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("sss", $param_room_id, $param_room_name,  $param_description);
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                  <?php
                      ...
                      try{  
                          $sql = 'SELECT room.room_id, room.room_name,room.description FROM room';
                         ...
                         if ($query->rowCount() > 0) {
                          ...
                          foreach ($dept as $row) 
                          {
                            ...
                            <tr>
                              ...
                              <td><?php echo htmlentities($row->room_name); ?></td>
                              <td><?php echo htmlentities($row->description); ?></td>
                              
                              <td>
                                <a href="#edit_<?php echo $row->room_id; ?>" class="btn btn-success btn-sm" data-toggle="modal"><span class="fa fa-edit "></span> Edit </a>
                                <a href="#delete_<?php echo $row->room_id; ?>" class="btn btn-danger btn-sm" data-toggle="modal"><span class="fa fa-trash"></span> Delete </a>
                              </td>
                             <?php include 'room_edit_delete_modal.php';?>
                            </tr>
                            <?php 
                            $i++;
                          }
              ...
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-9 <?php echo (!empty($room_id_err)) ? 'has-error' : ''; ?>">
                    <input type="text" name="room_id" class="form-control"  placeholder="Room Code">
                    <span class="help-block"><?php echo $room_id_err; ?></span>
                  </div>
                </div> 

                <div class="form-group ">
                  <label for="inputEmail3" class="col-sm-3 control-label"> Room Name </label>

                  <div class="col-sm-9 <?php echo (!empty($room_name_err)) ? 'has-error' : ''; ?>">
                    <input type="text" name="room_name" class="form-control"  placeholder="Room Code">
                      <span class="help-block"><?php echo $room_name_err; ?></span>
                </div>
              </div>

                <div class="form-group">
                  <label for="inputdept_head3" class="col-sm-3 control-label">Description </label>

                  <div class="col-sm-9 <?php echo (!empty($description_err)) ? 'has-error' : ''; ?>">
                    <input type="text" name="description" class="form-control"  placeholder="Room Code">
